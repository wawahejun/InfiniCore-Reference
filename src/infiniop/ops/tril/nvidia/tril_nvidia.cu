...
using op::tril::cuda::tril_kernel;
using op::tril::cuda::tril_kernel_inplace;
...
infiniStatus_t Descriptor::calculate(
    ...
    case INFINI_DTYPE_I8:
        tril_kernel<int8_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<int8_t *>(output),
            static_cast<const int8_t *>(input),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_I16:
        tril_kernel<int16_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<int16_t *>(output),
            static_cast<const int16_t *>(input),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_I32:
        tril_kernel<int32_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<int32_t *>(output),
            static_cast<const int32_t *>(input),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_I64:
        tril_kernel<int64_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<int64_t *>(output),
            static_cast<const int64_t *>(input),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_U8:
        tril_kernel<uint8_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<uint8_t *>(output),
            static_cast<const uint8_t *>(input),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_U16:
        tril_kernel<uint16_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<uint16_t *>(output),
            static_cast<const uint16_t *>(input),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_U32:
        tril_kernel<uint32_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<uint32_t *>(output),
            static_cast<const uint32_t *>(input),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_U64:
        tril_kernel<uint64_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<uint64_t *>(output),
            static_cast<const uint64_t *>(input),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_F16:
        tril_kernel<fp16_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<fp16_t *>(output),
            static_cast<const fp16_t *>(input),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_F32:
        tril_kernel<float><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<float *>(output),
            static_cast<const float *>(input),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_F64:
        tril_kernel<double><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<double *>(output),
            static_cast<const double *>(input),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_BF16:
        tril_kernel<bf16_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<bf16_t *>(output),
            static_cast<const bf16_t *>(input),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_BOOL:
        tril_kernel<bool><<<grid_size, block_size, 0, cuda_stream>>>(
...
infiniStatus_t Descriptor::calculateInplace(
    ...
    switch (input_dtype) {
    case INFINI_DTYPE_I8:
        tril_kernel_inplace<int8_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<int8_t *>(input_output),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_I16:
        tril_kernel_inplace<int16_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<int16_t *>(input_output),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_I32:
        tril_kernel_inplace<int32_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<int32_t *>(input_output),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_I64:
        tril_kernel_inplace<int64_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<int64_t *>(input_output),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_U8:
        tril_kernel_inplace<uint8_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<uint8_t *>(input_output),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_U16:
        tril_kernel_inplace<uint16_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<uint16_t *>(input_output),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_U32:
        tril_kernel_inplace<uint32_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<uint32_t *>(input_output),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_U64:
        tril_kernel_inplace<uint64_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<uint64_t *>(input_output),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_F16:
        tril_kernel_inplace<fp16_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<fp16_t *>(input_output),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_F32:
        tril_kernel_inplace<float><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<float *>(input_output),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_F64:
        tril_kernel_inplace<double><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<double *>(input_output),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_BF16:
        tril_kernel_inplace<bf16_t><<<grid_size, block_size, 0, cuda_stream>>>(
            static_cast<bf16_t *>(input_output),
            rows, cols, _diagonal);
        break;
    case INFINI_DTYPE_BOOL:
        tril_kernel_inplace<bool><<<grid_size, block_size, 0, cuda_stream>>>(
