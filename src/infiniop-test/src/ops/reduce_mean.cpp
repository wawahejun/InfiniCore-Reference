...
std::shared_ptr<infiniop_test::Result> Test::run(
    ...
    size_t dim_value = 0;
    if (_attributes->dim.size() >= sizeof(size_t)) {
        dim_value = *reinterpret_cast<const size_t*>(_attributes->dim.data());
    }
    ...
    CHECK_OR(infiniopCreateReduceMeanDescriptor(handle, &op_desc,
                                               ...
                                               input->desc(),
                                               dim_value),
             return TEST_FAILED(OP_CREATION_FAILED, "Failed to create reduce_mean descriptor."));
    ...
    infiniopDestroyReduceMeanDescriptor(op_desc);
    if (workspace) {
...
std::string Test::toString() const {
    ...
    if (_attributes->dim.size() >= sizeof(size_t)) {
        size_t dim_value = *reinterpret_cast<const size_t*>(_attributes->dim.data());
