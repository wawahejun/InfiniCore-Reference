...
def reference_triu(input: torch.Tensor, diagonal: int = 0) -> torch.Tensor:
    return torch.triu(input, diagonal=diagonal)
...
def test(shape, diagonal, dtype=torch.float32):
    """Generate a single test case for triu operation"""
    ...
    output_tensor = reference_triu(input_tensor, diagonal)
...
if __name__ == "__main__":
    ...
    print(f"Generated {len(test_cases)} test cases for Triu operator")
    test_writer.add_tests(test_cases)
    test_writer.save()
    print("Triu test cases saved to triu.gguf")