...
def tril_torch(input_tensor, diagonal=0):
    """PyTorch tril操作的参考实现"""
...
def test(handle, device, input_shape, diagonal, input_stride, output_stride, inplace=Inplace.OUT_OF_PLACE, dtype=InfiniDtype.F32, sync=None):
    ...
    output_shape = input_shape  # tril输出形状与输入相同
    
    print(
        f"Testing Tril on {InfiniDeviceNames[device]} with input_shape:{input_shape} diagonal:{diagonal} input_stride:{input_stride} output_stride:{output_stride} dtype:{InfiniDtypeNames[dtype]} inplace:{inplace.name}"
    )
    ...
    # PyTorch参考实现
    output_torch = tril_torch(input_tensor.torch_tensor(), diagonal)
    
    # 创建算子描述符
    descriptor = infiniopOperatorDescriptor_t()
    check_error(LIBINFINIOP.infiniopCreateTrilDescriptor(
        handle,
    ...
    def lib_tril():
        check_error(LIBINFINIOP.infiniopTril(
            descriptor, 
            None, 0, 
            output.data(), 
            input_tensor.data(), 
    ...
    if PROFILE:
        # fmt: off
        profile_operation("PyTorch", lambda: tril_torch(input_tensor.torch_tensor(), diagonal), torch_device, NUM_PRERUN, NUM_ITERATIONS)
        profile_operation("    lib", lambda: lib_tril(), torch_device, NUM_PRERUN, NUM_ITERATIONS)
        # fmt: on
